...
describe('buildLatencyPlot', () => {
  it('sorts pools by latest sample time by default and returns shared time and latency domains', () => {
    const plot = buildLatencyPlot([
      { id: 'a1', poolName: 'Pool A', timestampMs: 1_000, latencyMs: 20 },
      { id: 'b1', poolName: 'Pool B', timestampMs: 2_000, latencyMs: 40 },
      { id: 'a2', poolName: 'Pool A', timestampMs: 3_000, latencyMs: 80 },
    ], {
  ...
  it('can sort pools alphabetically', () => {
    const plot = buildLatencyPlot([
      { id: 'b1', poolName: 'Pool B', timestampMs: 2_000, latencyMs: 40 },
      { id: 'a1', poolName: 'Pool A', timestampMs: 1_000, latencyMs: 20 },
    ], {
