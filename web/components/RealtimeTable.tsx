...
// New prop integration: if filterBlockHeight is provided, fetch mining_notify data for that block height.
// When filterBlockHeight is undefined, subscribe to realtime SSE events.
export default function RealtimeTable({ filterBlockHeight }: { filterBlockHeight?: number }) {
  // SSE rows
  const [rows, setRows] = useState<MiningData[]>([]);
  // Reference for SSE subscription
  ...
  // When filterBlockHeight becomes undefined, clear rows to allow realtime SSE streaming.
  useEffect(() => {
    if (filterBlockHeight === undefined) {
      setRows([]);
    }
  }, [filterBlockHeight]);
  ...
  // SSE subscription or API fetch based on filterBlockHeight
  useEffect(() => {
    if (filterBlockHeight !== undefined) {
      ...
      fetch(`/api/mining-notify?height=${filterBlockHeight}`)
        ...
        .catch((err) => {
          console.error("Error fetching mining_notify data:", err);
          setRows([]);
        });
      return;
    }
    ...
    <div className="p-4 font-mono w-full overflow-x-auto relative">
      ...
      <div className="flex justify-end items-center mb-2 space-x-4">
        ...
        {showSettings && (
          <div
            ref={settingsRef}
          className="settings-dropdown absolute top-10 right-4 bg-white dark:bg-[#1e1e2f] border border-gray-300 dark:border-gray-700 shadow-md rounded p-2 z-50"
          >
          <div>
            <ModeToggle />
